...
authRoutes.post('/register',
  ...
  async (c) => {
    ...
    // Set cookies
    c.res.headers.append('Set-Cookie',
...
authRoutes.post('/refresh', async (c) => {
  ...
  c.res.headers.append('Set-Cookie',
    `accessToken=${newAccessToken}; HttpOnly; Secure; Path=/; SameSite=Strict`
  );
...
authRoutes.post('/logout', async (c) => {
  // Clear auth cookies
  c.res.headers.append('Set-Cookie',
    'accessToken=; HttpOnly; Secure; Path=/; SameSite=Strict; Expires=Thu, 01 Jan 1970 00:00:00 GMT'
  );
  c.res.headers.append('Set-Cookie',
    'refreshToken=; HttpOnly; Secure; Path=/; SameSite=Strict; Expires=Thu, 01 Jan 1970 00:00:00 GMT'
  );

  return c.json({ message: 'Logged out successfully' });
});

authRoutes.post('/request-reset',
  resetRateLimiter,
  zValidator('json', z.object({ email: z.string().email() })),
  async (c) => {
    const { email } = c.req.valid('json');
